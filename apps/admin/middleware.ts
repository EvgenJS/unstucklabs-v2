...
// same header and stamps it onto those scripts automatically. Confirmed on
// Store by testing: without the nonce, those inline scripts get blocked and
// the app hydrates to a blank page.
export function middleware(request: NextRequest) {
  const nonce = btoa(crypto.randomUUID());
  const cspHeader = [
    `default-src 'self'`,
    `script-src 'self' 'nonce-${nonce}' 'strict-dynamic'${isDev ? " 'unsafe-eval'" : ""}`,
    // No next/image `fill` usage in Admin, so style-src doesn't need
    // 'unsafe-inline' the way Store's does.
    `style-src 'self'`,
